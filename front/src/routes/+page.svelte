...
$: {
		...
		if (browser) {
			if (checked === null) {
				const currentTheme = (window.localStorage.getItem('theme') ?? 'dark').toLowerCase();
...
<div class="bg-green-400 w-32 h-32 rounded-xl"></div>

<p class="text-3xl text-red-500 underline">test</p>
